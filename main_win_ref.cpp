...
void writeStringAt(HANDLE *handle, SHORT x, SHORT y, std::string s) {
	COORD pos = {x, y};
	SetConsoleCursorPosition(*handle, pos);
			...
			switch(inputRecBuffer[i].EventType) {
				case KEY_EVENT:
					...
					if (keyIsDown != keyWasDown) {
						if (keyIsDown) {
							writeStringAt(&hOut, 5, 5, "KEY IS DOWN! ");
							writeCharAt(&hOut, 5, 6, ' ');
							std::cout << "ascii: " << keyEvent.uChar.AsciiChar << std::flush;
							writeCharAt(&hOut, 5, 7, ' ');
							...
							writeStringAt(&hOut, 5, 5, "KEY IS UP!          ");
							writeStringAt(&hOut, 5, 6, "                    ");
							writeStringAt(&hOut, 5, 7, "                    ");
						}
					}
